...
def main(args=None, parser=None):
	...
	##indexFile = 'color_labels.txt'
	indexFile = 'depth_labels.txt'

	# Let's collect and write out the training and validation files
	##ofstrain = open('color_train.txt', 'w')
	##ofsval = open('color_val.txt', 'w')	
	##ofstest = open('color_test.txt', 'w')
	ofstrain = open('depth_train.txt', 'w')
	ofsval = open('depth_val.txt', 'w')	
	ofstest = open('depth_test.txt', 'w')
